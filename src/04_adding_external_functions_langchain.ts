...
const ObterTemperaturaAtualSchema = z.object({
  local: z
    .string()
    .min(1, { message: "O nome da cidade deve conter pelo menos um caractere" })
    .describe("O nome da cidade"),
  unidade: UnidadeEnum.optional().describe(
...
async function exemploComBind() {
  try {
    ...
    const resposta = await chatComFunc.invoke(
...
async function exemploForcandoFuncao() {
  try {
    ...
    const resposta = await chat.invoke("Olá", {
      ...
      tool_choice: {
        type: "function",
        function: { name: "obterTemperaturaAtual" },
      },
    });
...
const GetEmailsSchema = z.object({
  search: z
    .string()
    .min(1, { message: "The search query must be at least 1 character" })
    .describe("Query to search for"),
  inbox: inboxEnum
    .optional()
    .describe("Inbox (unread, read, starred, unstarred)"),
});
...
const getEmails = (search: string, inbox: string = "unread"): string => {
  ...
  if (search.toLowerCase().includes("tecnologia") && inbox === "read") {
    ...
    search.toLowerCase().includes("tecnologia") &&
    inbox === "unread"
  ) {
...
async function challenge01Case01() {
  try {
    ...
    const resposta = await chain.invoke({
      input: "Quais são os emails do inbox unread sobre tecnologia",
    });
...
async function challenge01Case02() {
  try {
    ...
    const resposta = await chain.invoke({
      input: "Quais são os emails do inbox read sobre tecnologia",
    });
...
async function challenge01Case03() {
  try {
    ...
    const resposta = await chain.invoke({
      input: "Quais são os emails do inbox starred sobre tecnologia",
    });
